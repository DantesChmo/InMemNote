...
test('markdown source styles headings and inline formatting', async () => {
  ...
  try {
    ...
    const measurements = await draft.raw.evaluate(() => {
      ...
      // Measure the WHOLE .cm-line: heading sizing now lives on the line via
      // the `cm-inmem-h*` decorations, not on individual highlight spans.
      return {
    ...
    // Caret is on the `## Subheading` line; the `# ` marker on the (now
    // inactive) `# Heading` line must be removed from the visual flow
    // entirely — replace-decoration drops the range from the layout, so the
    // line's `textContent` no longer starts with `#`.
    ...
    // And the converse: clicking back into the `# Heading` line must bring
    // its marker back so the user can edit the syntax.
